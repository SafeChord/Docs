...
| **🧠 維護者** | Repo 理解力與長流程專注度；沒有別的東西會攔下它的錯 | Claude Code · **Opus 5 / high effort** | **`Docs/` 的持有者**，兩個方向都管。開票之前：撰寫藍圖或設計草稿，然後開票並上標籤。開票之後：程式碼審查、測試規劃、文件對齊，以及推動[發版流程](safechord.safezone.delivery.workflow.md)。 |

**能力要求是契約，當前載體是實作細節。** 換掉載體，不要動席位。

**`Docs/` 屬於維護者。** 開拓者在那裡是唯讀。實作過程中發現的偏離，先把程式碼做完，再交接出去做文件對齊。

**席位綁定在 session 上。** 一個 session 持一個席位，持到結束。換席位就是換 session，用交接把 context 帶過去。

**預設席位是維護者。** 開拓者由人類在 session 開頭宣告。未宣告的 session 不寫實作程式碼——先問。

### 1.2 審查跑在自己的 session

**程式碼審查不得與它所審查的實作共用同一個 session**——即使兩個席位跑在同一個載體上。

範圍只到審查。不延伸到維護者的其他職責：文件對齊與測試規劃可以跑在持有 context 的那個 session 裡。
...
1.  **策略設計**: 人類定義「Why/What」；維護者更新 Markdown 知識地圖（藍圖/ADR），接著**開票並標記 `kdd:forward`**。
...
4.  **鞏固**: **關鍵階段。** 維護者執行 PR 審查並進行**文件對齊**，將 Spike 結果逆向工程回 `Docs/` 中的單一事實來源（SSOT）。**把票結算成新的專案文件並關票。**
